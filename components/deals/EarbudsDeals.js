...
export default function EarbudsDeals() {
    ...
    function getData() {
        axios.post('https://server-liard-zeta.vercel.app/earphones', { 'range': 50000 }).then((res) => {
            setData(res.data);
        })
    }
    ...
    return (
        <div className="container w-full flex flex-col items-center justify-center gap-10">
            ...
            <h1 className="text-xl md:text-5xl font-bold uppercase my-10 text-gray-700">earphones</h1>
            ...
            <div className="container h-[470px] w-full overflow-x-auto columns-1 md:columns-4 over-x-scroll">

                {
                    data?.map((d, i) => {
                        while (i < 10) {
                            return (
                                <ProductCard key={i} d={d} category={'earphones'}/>
                            )
                        }
                    })
                }
